...
def collect_image_paths(data_dir):
    ...
    crawler_ = futil.FileSystemDataCrawler(data_dir, image_keys, MyFilePathGenerator(), dir_filter, '.nii.gz')  # todo: modify here
...
def load_images(image_paths):
    ...
    image_dict = {
        structure.BrainImageTypes.T1w: sitk.ReadImage(image_paths.__getitem__(structure.BrainImageTypes.T1w), sitk.sitkFloat32), # todo: modify here
        structure.BrainImageTypes.GroundTruth: sitk.ReadImage(image_paths.__getitem__(structure.BrainImageTypes.GroundTruth), sitk.sitkUInt8) # todo: modify here
    }
...
def register_images(image_dict, atlas_img):
    ...
    registered_gt = sitk.Resample(gt_img, referenceImage=atlas_img, transform = registration.transform, interpolator = sitk.sitkNearestNeighbor)  # todo: modify here
...
def preprocess_filter_rescale_t1(image_dict, new_min_val, new_max_val):
    ...
    minmax_rescaled_img = filter_.execute(image_dict.get(structure.BrainImageTypes.T1w), filter_params)  # todo: modify here
...
def extract_feature_median_t1(image_dict):
    ...
    median_img = filter_.execute(image_dict.get(structure.BrainImageTypes.T1w))  # todo: modify here
